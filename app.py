...
# Load .env once at entry point — all modules inherit the environment
load_dotenv()
...
# ── Full restart handler — runs before any routing ─────────────────────────────
...
# ── ROUTER — Flow: intro → act1 → act2 → act3 → closing ───────────────────────
if st.session_state.current_act == "intro":
    ...
    show_act2()
elif st.session_state.current_act == "act3":
    from act3_mirror.mirror import show_act3
    show_act3()
